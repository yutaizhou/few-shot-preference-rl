...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", "-o", type=str, default="plot.png", help="Path of output plot"
    )
    parser.add_argument(
        "--path", "-p", nargs="+", type=str, required=True, help="Paths of runs to plot"
    )
    parser.add_argument(
        "--legend",
        "-l",
        ...
        type=str,
        required=False,
        help="Names of each run to display in the legend",
    )
    parser.add_argument("--title", "-t", type=str, required=False, help="Plot title")
    parser.add_argument(
        "--window", "-w", type=int, default=1, help="Moving window averaging parameter."
    )
    parser.add_argument("--x", "-x", type=str, default="step", help="X value to plot")
    parser.add_argument(
        "--max-x", "-m", type=int, default=None, help="Max x value to plot"
    )
    parser.add_argument(
        "--xlabel", "-xl", type=str, default=None, help="X label to display on the plot"
    )
    parser.add_argument(
        "--y",
        "-y",
        ...
        nargs="+",
        default=["eval/loss"],
        help="Y value(s) to plot",
    )
    parser.add_argument(
        "--ylabel", "-yl", type=str, default=None, help="Y label to display on the plot"
    )
    ...
    if len(paths) == 1 and paths[0].endswith(".yaml"):
        ...
        if len(paths) == 1 and all(
            [os.path.isdir(os.path.join(paths[0], d)) for d in os.listdir(paths[0])]
        ):
        ...
        if labels is None:
            labels = [
                os.path.basename(path[:-1] if path.endswith("/") else path)
                for path in paths
            ]
        # Sort the paths alphabetically by the labels
        paths, labels = zip(
            *sorted(zip(paths, labels), key=lambda x: x[0])
